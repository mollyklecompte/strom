...
class TestPandaDB(unittest.TestCase):
    def setUp(self):
        ...
        self.db.connect()
        self.daf = pandas.DataFrame({"name":['aj', 'as', 'aj'], "version":[0.5, 1.5, 2.0]}, index=[0,1,2])# replace with actual data
    ...
    # def test_create(self):# NOTE
    #     self.db.table(df=self.daf, table='test', action='replace')
    #     newdaf = pandas.DataFrame({"name": "test", "version": 0.1, "extra":5}, index=[0])
    #     self.db.create(df=newdaf, table='test')
    #     res = self.db.select(table='test')
    #     del res['index']
    #     del res['level_0']
    #     ores = self.daf.append(newdaf)
    #     ores.reset_index()
    #     res.reset_index()
