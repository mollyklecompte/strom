...
class Coordinator(object):
    ...
    def _store_json(self, data, data_type):
        ...
        :type data: dict
        :param data_type: kind (not formal type) of data to be stored-
         "template", "derived", or "event"
        :type data_type: string
        :return: unique id of document in Mongo
        (generated by Mongo on insert)
        :rtype: string
        """
    ...
    def _store_raw(self, bstream):
        ...
        :rtype: int
        """
        rows_inserted = self.maria.insert_rows_into_stream_lookup_table(bstream)

        return rows_inserted

    def _store_filtered(self, bstream):
        """
        Passes token, timestamp list, filtered measures
        to maria manager for storage.
        ...
        rows_inserted = self.maria.insert_rows_into_stream_filtered_table(
            filtered_dict)
    ...
    def _retrieve_current_template(self, token):
        ...
        temp_id = ObjectId(self.maria.return_template_id_for_latest_version_of_stream(token))
    ...
    def _retrieve_data_by_timestamp(self, dstream, ts):
        """
        ...
        if isinstance(ts, int) or isinstance(ts, float):
            return self.maria.retrieve_by_timestamp_range(dstream, ts, ts)
        #  if time is an array or tuple (time range)
        else:
            start = ts[0]
            end = ts[1]
            return self.maria.retrieve_by_timestamp_range(dstream, start, end)
    ...
    def _store_raw_old(self, data_list):
        ...
        for dstream in data_list:
            row_id = self.maria.insert_row_into_stream_lookup_table(dstream)
            ids.append(row_id)
    ...
    def _store_filtered_old_dumb(self, bstream):
        """
        When we foolishly UPDATED rows in stream table with filtered data.
        ...
        for measure, val_id_pair_list in zippies.items():
            for i in val_id_pair_list:
                val = i[0]
                v_id = i[1]
                self.maria.insert_filtered_measure_into_stream_lookup_table(token,
                                                                            measure,
                                                                            str(val),
                                                                            v_id)
